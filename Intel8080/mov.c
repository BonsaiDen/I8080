...
// Move FROM to TO
#define MOV(TO, FROM) static void MOV_##TO##_##FROM(Intel8080 *cpu) { \
    *cpu->TO = *cpu->FROM; \
}

// Move BYTE (HL) to TO
#define MOV_M_R(TO) static void MOV_##TO##_M(Intel8080 *cpu) { \
...
// Write FROM to (HL)
#define MOV_M(FROM) static void MOV_M_##FROM(Intel8080 *cpu) { \
...
// Move HL to SP
static void SPHL(Intel8080 *cpu) {
    *cpu->SP = *cpu->HL;
}

// Load next WORD into REG
#define LXI(NAME, REG) static void LXI_##NAME(Intel8080 *cpu) { \
